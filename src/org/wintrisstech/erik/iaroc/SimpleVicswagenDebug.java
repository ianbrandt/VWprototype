...
public final class SimpleVicswagenDebug implements VicswagenInterface {
	...
	private static final float PULSE_LENGTH_TO_DISTANCE = 18000F;
	/* Instance variables */
	...
	/* The following variables require calibration */
    private float angularVelocityFactor = 0.00312F; //
    private float velocityFactor = 0.545F;
	/**
	 * Must be true for motors to run. high = slow decay, low = fast decay
	 */
	...
	public SimpleVicswagenDebug(IOIO ioio, Dashboard dashboard)
	...
	private void initialize() throws ConnectionLostException {
		...
		frontDistanceInput = ioio.openPulseInput(FRONT_ULTRASONIC_INPUT_PIN,
				PulseInput.PulseMode.POSITIVE);
		leftDistanceInput = ioio.openPulseInput(LEFT_ULTRASONIC_INPUT_PIN,
				PulseInput.PulseMode.POSITIVE);
		rightDistanceInput = ioio.openPulseInput(RIGHT_ULTRASONIC_INPUT_PIN,
				PulseInput.PulseMode.POSITIVE);
		// Single precision for the rear sensor because there is maximum 3
		// double precision modules.
		rearDistanceInput = ioio.openPulseInput(new DigitalInput.Spec(
				REAR_ULTRASONIC_INPUT_PIN), PulseInput.ClockRate.RATE_2MHz,
		...
		dashboard.log("Enabling motors");
		motorEnable = ioio.openDigitalOutput(MOTOR_ENABLE_PIN);
		motorEnable.write(true);
		rightDirection = ioio.openDigitalOutput(RIGHT_DIRECTION_OUTPUT_PIN);
		leftDirection = ioio.openDigitalOutput(LEFT_DIRECTION_OUTPUT_PIN);
	}
	
	public void calibrateVelocity(float factor) {
    	velocityFactor = factor;
    }
    
    public void calibrateAngularVelocity(float factor) {
    	angularVelocityFactor = factor;
    }

	public void driveDirect(int leftVelocity, int rightVelocity)
			throws ConnectionLostException {
		dashboard.log("VELOCITY_TO_FREQUENCY = " + velocityFactor);
		dashboard.log("leftVelocity = " + leftVelocity);
		dashboard.log("rightVelocity = " + rightVelocity);
		leftDirection.write(leftVelocity < 0);
		rightDirection.write(rightVelocity < 0);
		int leftFrequency = (int) (Math.abs(leftVelocity) * velocityFactor);
		int rightFrequency = (int) (Math.abs(rightVelocity) * velocityFactor);
		dashboard.log("leftFrequency = " + leftFrequency);
		dashboard.log("rightFrequency = " + rightFrequency);
		...
		leftPwmOutputOpen = true;
		rightPwmOutput = ioio.openPwmOutput(RIGHT_PWM_OUTPUT_PIN,
				rightFrequency);
		rightPwmOutputOpen = true;
	...
	public int getAngle() {
		...
		float angularVelocity = (rightVelocity - leftVelocity) * angularVelocityFactor;
		float angleRadians = (SystemClock.elapsedRealtime() - startTime)
	...
	public int getTurnRadius() {
		...
		return (int) ((leftVelocity + rightVelocity)
				/ (rightVelocity - leftVelocity) / angularVelocityFactor / 2);
	}
	...
	public int getFrontUltrasonicDistance() throws InterruptedException {
		try {
			...
			frontStrobe.write(false);
			frontDistance = (int) (frontDistanceInput.waitPulseGetDuration() * PULSE_LENGTH_TO_DISTANCE);
	...
	public int getRearUltrasonicDistance() throws InterruptedException {
		...
		try {
			...
			rearStrobe.write(false);
			rearDistance = (int) (rearDistanceInput.waitPulseGetDuration() * PULSE_LENGTH_TO_DISTANCE);
		} catch (ConnectionLostException ex) {
		}
		return rearDistance;
	}
